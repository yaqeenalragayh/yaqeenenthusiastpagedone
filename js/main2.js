(function ($) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        profileIcon.addEventListener('click', function(e) {
            e.stopPropagation();
            DM2.classList.toggle('active');
        });
        
        // Close DM2 when clicking outside
...
menuItems.forEach(item => {
    item.addEventListener('click', function(e) {
        // Skip if this is the edit profile link
        const profileLink = this.querySelector('a[href="EnthusiastProfilePage2.php"]');
        if (profileLink) {
...
function handleMenuItemClick(action) {
    ...
    switch(action) {
        ...
        case 'messages':
            ...
            break;
        case 'edit-profile':
            // This won't be reached because we already skipped the ProfilePage2 link
            window.location.href = 'EnthusiastProfilePage2.php'; 
            break;
        case 'logout':
    ...
    document.querySelector('[data-action="edit-profile"] a').addEventListener('click', function(e) {
        console.log('Edit profile link clicked');
        // If you see this log but no navigation happens, there might be a JS error
    });
    //profile end
